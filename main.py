...
def main():
    ...
    qtd_horas_total = st.sidebar.number_input("Qtd. de horas totais do dia para trabalhar",
    ...
    minuto_inicial= colunas[1].number_input("Minuto final do dia (0-59)",
    ...
    df_nad, df_tabela_ligantes = gera_tabelas(
        qtd_dias_total, minutos_dividir, hora_inicial, minuto_inicial,
        turnos, encargos
    )
    ...
    arquivo_duracoes = st.sidebar.file_uploader(
        "Carregar arquivo de duracoes preenchidas",
        type=["xlsx"]
    )

    if arquivo_duracoes is not None:
        df_duracoes = pd.read_excel(arquivo_duracoes)
    arquivo_ligantes = st.sidebar.file_uploader("Carregara arquivo de ligantes por preenchidos",
                                                type=["xlsx"])
    ...
    opcao = st.sidebar.segmented_control(
        ...
        label="Escola a opcao",
        default="Rodar Attentus",
        selection_mode="single"
    )
    botao = st.sidebar.button("Avancar")
